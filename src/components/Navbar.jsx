...
const Navbar = () => {
    ...
    const [token, setToken] = useState(true); // when ever we have token it means we are logedin  
                    ...
                    token ? <div className='flex items-center gap-2 cursor-pointer relative group'>
                        ...
                        <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                            <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4'>
                               ...
                               <p onClick={()=>navigate('my-appointment')} className='hover:text-black coursor-pointer'> My Appoitment </p>
                               <p onClick={()=> setToken(false)} className='hover:text-black coursor-pointer'> Logout </p>
                            </div>
                        </div>
                    </div> :
                        <button onClick={() => navigate("/log-in")} className='bg-primary text-white px-8 py-3 rounded-full font-light hidden md:block'>
                            Create Account
                        </button>
                }
                <img onClick={()=>setShowMenu(true)} className='w-6 md:hidden' src={assets.menu_icon} alt='' />
                {/* mobile menu */}
                <div className={` ${showManu?'fixed w-full': `h-0 w-0` } md:hidden right-8 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
