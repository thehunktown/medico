  ...
  return (
    <form className='min-h-[80vh] flex items-center '>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        <div className='w-full'>
          <p>Phone</p>
          <input className='border border-zinc-300 rounded w-full p-2 mt-1' type='phone' onChange={(e) => setPhone(e.target.name)} value={phone} required/>
        </div>
        <div className='w-full'>
          <p>Password</p>
        ...
        {
          state === 'Sign Up'? <p>Already have account <span  onClick={()=> setState('Login')} className='text-primary underline cursor-pointer'> login here </span></p>: <p>Create an new account? <span onClick={()=> setState('Sign Up')} className='text-primary underline cursor-pointer'> click here </span> </p>
        }
      </div>
    </form>
  )
